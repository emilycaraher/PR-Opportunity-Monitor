...
| `digest-public.json` | Sample digest output with anonymized client labels (Client 1–4) for live demo use |
...
Retrieval evaluated using leave-one-out consistency: for each article, its title is used as a query against the remaining corpus. A hit is counted when at least one result scores >= 0.55. Scores increased monotonically with alpha for all four clients, confirming dense-only retrieval as the optimal configuration.
...
**To preview without running the notebook:** download `digest-public.json` and `pr_monitor.html`, then load the JSON file using the upload button in the UI.
